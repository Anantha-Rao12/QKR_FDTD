    ...
    def get_kickingstrength(self, time: int) -> float:
        ...
        return (
            ...
            * (
                1
                + self.eps
                * np.cos(self.w3 * time + self.phi2)
                * np.cos(self.w2 * time + self.phi3)
            )
            / self.hbar
        )

    def get_kickingstrength_normal(self, time: int, k_sigma: float) -> float:
        noisy_K = np.random.randn(1) *k_sigma +  self.K0
        return noisy_K/ self.hbar

    def get_positionspace_unitary(self, time: int, form, kick_sigma) -> np.ndarray:
        ...
        if form == "QPKR":
            ...
            return np.exp(
                -1j * self.get_kickingstrength_normal(time, kick_sigma) * np.cos(self.x)
            )

    def evolve(self, form, *k_sigma):
        ...
        for t in range(self.total_time):
            Ux = self.get_positionspace_unitary(t, form, k_sigma)
